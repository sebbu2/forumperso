...
if( isset($name) && isset($contenu) && isset($sujet) && $name!='' && $contenu!='' && $sujet!='') {
 ...
 $sujet1=$ligne['sujet'];
 $tid=$ligne['tid'];
 print('<h2><span class="top"><a href="viewforums.php?id='.$fid.'">'.$forum.'</a> > <a href="viewtopics.php?id='.$tid.'">'.$sujet1.'</a></span></h2>');
 $date=date('d/m/Y');
 $time=date('H:i:s');
 $contenu2=$contenu;
...
// $contenu2=addslashes(strip_tags($contenu2));
 ...
 //die($sql);
 $sujet2=$sujet;
 $sujet2=changer_specials_chars($sujet2,0);
// $sujet2=addslashes(strip_tags($sujet2));
 $res=send_sql($sql);
 $sql2='UPDATE `forums_topics` SET `sujet`="'.$sujet2.'" WHERE `fid`="'.$fid.'" AND `tid`="'.$tid.'";';
 //die($sql2);
 $res2=send_sql($sql2);
 if($res!='1' || $res2!='1') print('&eacute;chec');
 if($res=='1' && $res2=='1') print('<p>Le message a bien &eacute;t&eacute; &eacute;dit&eacute;.</p>');
}
else {
/*        Pseudo<br>
...
// $message=stripslashes($message);
 ...
 print('      <form action="'.$PHP_SELF.'?id='.$id.'" method="post" id="post_text"><div>
		...
		<br/>Sujet<br/><input type="text" name="sujet" size="81" value="'.$sujet.'"/><br/>
        Message
        <br/>');
 top_smile('text');
