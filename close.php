...
$sql='SELECT p3.`fid`, p3.`tid`, p2.`titre`, p3.`sujet`, p2.`moderator`'.
' FROM `forums_title` AS p2, `forums_topics` AS p3 WHERE p3.`fid`=p2.`fid` and p3.`tid`="'.$id.'";';
$res=send_sql($sql);
...
$sql='UPDATE `forums_topics` SET `status`="close" WHERE `tid`="'.$id.'";';
$res=send_sql($sql);
if($res==1) print('<h2>Sujet ferm&eacute;.</h2>');
if($res!=1) print('<p>&eacute;chec lors de la fermeture du sujet.</p>');
